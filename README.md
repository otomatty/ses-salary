...
四半期で決まる「基本給」に、月ごとの **残業代** と **特別手当**（役職手当など）を
加算して、当月の **額面実支給見込み** を算出する（`src/shared/income.ts`）。
ホームの今期カードに「実支給見込み（当月）」として加算表示する。

- **残業代** = `(基本給 + 職務手当) ÷ 月平均所定労働時間 × 割増率`（会社規程準拠）。
  みなし残業（固定時間外手当）を **超えた分のみ** 加算する（二重計上しない）。
  割増率は通常 1.25 / 月60時間超 1.5 / 深夜 +0.25 / 法定休日 1.35。
  残業時間は月単価ページで月ごとに入力（通常残業が主入力。深夜・法定休日は任意）。
  API は `POST /api/overtime`（年月で upsert）。
- **特別手当** は名前付きで **適用開始月** ごとに登録し、その月以降に継続適用する
  （評価ランク履歴と同じ「最新が有効」方式。金額 0 で廃止）。職務手当など
  残業単価の基礎に算入する手当は「残業基礎に含める」を有効にする。
  設定画面で管理。API は `POST /api/allowances`。
- **雇用形態・月平均所定労働時間** は設定画面で登録する（API は `POST /api/settings`）。
  みなし残業時間は雇用形態から決まる（正社員エンジニア 20h / 契約アカデミア 14h /
  契約コーポレート 40h / その他 0h）。
- 残業・手当は **帯／還元率（単価ベース）の判定には影響しない**（基本給の上に加算するだけ）。
